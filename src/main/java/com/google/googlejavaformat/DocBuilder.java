...
public final class DocBuilder {
	 ...
	 * {@code a = b}, with an optional {@link Doc.Break} following the {@code =}. Suppose we have 5 characters to write
	 * it, so that we think we don't need the break. Unfortunately, this {@link Doc} lies in an expression statement
	 ...
	 * problems.
	 *
	 * <p>One solution to this problem is not to decide whether a {@link Doc.Level} should be broken until later (in
	 * this case, after the semicolon has been seen). A simpler approach is to rewrite the {@link Doc} as here, so that
	 * the semicolon moves inside the inner {@link Doc}, and we can decide whether to break that {@link Doc} without
	 * seeing later text.
	 */
