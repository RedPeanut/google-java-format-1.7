...
/**
 * {@link com.google.googlejavaformat.java.JavaInputAstVisitor JavaInputAstVisitor} outputs a sequence of {@link Op}s
 * using {@link OpsBuilder}. This linear sequence is then transformed by {@link DocBuilder} into a tree-structured
 * {@code Doc}. The top-level {@code Doc} is a {@link Level}, which contains a sequence of {@code Doc}s, including other
 * {@link Level}s. Leaf {@code Doc}s are {@link Token}s, representing language-level tokens; {@link Tok}s, which may
 * also represent non-token {@link Input.Tok}s, including comments and other white-space; {@link Space}s, representing
 * single spaces; and {@link Break}s, which represent optional line-breaks.
 */
public abstract class Doc {
	...
	public enum FillMode {
		...
		/**
		 * If a {@link Level} will not fit on one line, its {@code INDEPENDENT} {@link Break}s will be broken
		 * independently of each other, to fill in the {@link Level}.
		 */
		INDEPENDENT,

		/**
		 * A {@code FORCED} {@link Break} will always be broken, and a {@link Level} it appears in will not fit on one
		 * line.
		 */
		FORCED
	}
